...
namespace Org.Brotli.Dec
{
	...
	public class BrotliInputStream : System.IO.Stream
	{
		...
		public BrotliInputStream(System.IO.Stream source, int byteReadBufferSize, byte[] customDictionary)
		{
			...
			{
				throw new System.IO.IOException("Brotli解码器初始化失败", ex);
			}
		...
		public override int Read(byte[] destBuffer, int destOffset, int destLen)
		{
			if (destOffset < 0)
			{
				throw new System.ArgumentException("错误的偏移量: " + destOffset);
			}
			else if (destLen < 0)
			{
				throw new System.ArgumentException("错误的长度: " + destLen);
			}
			...
			{
				throw new System.IO.IOException("Brotli流解码失败", ex);
			}
		}
		// <{[INJECTED CODE]}>
